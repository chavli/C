  ...
  char *filename = "node";
	...
	while (num_forked < num_procs) {
        ...
        // Child initializes self (which creates a fifo) and waits for communication
		    startDistNode(initializeDistNode(getpid(), drift));
        exit(0);
		}
		else {
    ...
    printf("\n%d-node system finished executing after %d seconds.\n", num_procs, RUN_TIME);
    FILE *fd;
    ...
    fclose(fd);
		...
		for (i = 0; i < num_procs; ++i)
			sendMessage(atoi(*(pids + i)), CMD_EXIT, filename, 0);
    ...
    while(fgets(line, 128, fd)){
     sscanf(line, "%d %d", &pid, &ms);
     avg += ms;
     printf("NODE: %d -- elapsed seconds %f\n", pid, (ms / 1000.0));
    }
    fclose(fd);
    finaltime = start + ((avg / num_procs) / 1000);
    time = gmtime(&finaltime);
    printf("average system time: %s", asctime(time));
    finaltime = start + ( RUN_TIME / 1000);
    time = gmtime(&finaltime);
    printf("real time: %s", asctime(time));
	}
    ...
    if(strlen(line) > 1){
      ...
      if( strcmp(command, "exit") == 0){
        if ( time(NULL) - start < RUN_TIME )
          for (i = 0; i < num_procs; ++i)
            sendMessage(atoi(*(pids + i)), CMD_EXIT, 0, 0);
        break;
      }
